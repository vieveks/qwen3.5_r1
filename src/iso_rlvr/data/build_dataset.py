...
def build_dataset(out: Path, families: int, variants: int, seed: int) -> None:
    rng = random.Random(seed)
    out.parent.mkdir(parents=True, exist_ok=True)
    with out.open("w", encoding="utf-8") as handle:
        for family_idx in range(families):
            family_id = f"fam_{family_idx:06d}"
            for variant in make_family(rng, family_id, variants):
...
def main() -> None:
    ...
    build_dataset(args.out, args.families, args.variants, args.seed)
